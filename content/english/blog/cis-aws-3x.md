...
This blog-post refers especially to the “Monitoring” chapter of the **CIS AWS Foundations Benchmark** which contains the security controls **CIS AWS 3.1 - CIS AWS 3.14**. 
...
We fully agree to the goals of CIS AWS 3.x but we think that only ensuring a metric filter and an alarm exists for those controls is **not sufficient**. 
...
To find the alarm-causing event information you need to dig through the Amazon CloudWatch Logs and find the right event signature, which is cumbersome. 
